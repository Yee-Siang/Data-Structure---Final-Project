...
user_ls_out = []
for j in range(rows):
    individual = user(df.iat[j,3], int(df.iat[j,2]), df.iat[j,0], df.iat[j,1].split(' '), df.iat[j,4], df.iat[j,5])
    print(individual.name, individual.id)
    user_ls.append(individual)
    user_ls_out.append({'id':individual.id, 'name':individual.name, 'age':individual.age, 'gender':individual.gender, 'interest':individual.interest, 'ideal_age':individual.ideal_age, 'ideal_gender':individual.ideal_gender, 'paired':individual.paired})
